...
def register_user():        # 회원 정보 기록 함수
    ...
    Label(register_screen, text="회원가입 완료", fg="green", font=("맑은 고딕",11)).pack()
...
def login_success():            # 로그인 성공 창 함수
    ...
    login_success_screen.title("로그인")
    login_success_screen.geometry("150x100")
    Label(login_success_screen, text="로그인 성공").pack()

    Button(login_success_screen, text="OK",command=delete_login_success).pack()


def delete_login_success():         # 로그인 성공 창 삭제 함수
...
def password_not_recognized():      # 비밀번호 오류 창 함수
    ...
    password_not_recog_screen.title("로그인")
    password_not_recog_screen.geometry("150x100")
    Label(password_not_recog_screen,text="잘못된 비밀번호").pack()
    Button(password_not_recog_screen,text="OK",command=delete_password_not_recognized).pack()

def delete_password_not_recognized():   # 비밀번호 오류 창 삭제 함수
...
def user_not_found():                   # 아이디 오류 창 함수
    ...
    user_not_found_screen.title("로그인")
    user_not_found_screen.geometry("150x100")
    Label(user_not_found_screen,text="존재하지 않는 아이디").pack()
    Button(user_not_found_screen,text="OK",command=delete_user_not_found_screen).pack()

def delete_user_not_found_screen():    # 아이디 오류 창 삭제 함수
